...
\begin{tabular}{|l|l|}
	...
	Particle count & Number of particles in the swarm \\ \hline
\end{tabular}\\
...
Wang, Wang, Fu and Zhe present a new novel probability update strategy for the binary discrete PSO \cite{bib-novel}. The extended PSO is called probability binary particle swarm optimization algorithm (PBPSO) and prevents particles to get trapped in local optima. The update strategy uses a linear Function $F(x)$ to determine if a position $x_i$ in dimension $d$ is 1 or 0. Additionally to its better optimization results, this method prevents the use of the sigmoid function which reduces the required computation time.

\begin{equation}
L(x) = (x_i - R_{min} ) / (R_{max} - R_{min})
\end{equation}

In PBPSO a particles position $x_i$ consist now of real values (no binary values). $R_{max}$ and $R_{min}$ are pre-defined constants. In each iteration, the position $x_i$ is updated according to the normal formula $x_i = x_i + v_i$. Finally, the particles binary position $px_i$ (and therefore the fitness value to calculate pBest and gBest) is updated according to the following formula.

\begin{equation}
px_i = \left\{ 
\begin{array}{l l}
  1 & \quad \text{$rand() <= L(x_i)$}\\
  0 & \quad \text{otherwise}
\end{array} \right.
\end{equation}
